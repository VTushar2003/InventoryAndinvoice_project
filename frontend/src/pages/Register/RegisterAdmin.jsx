...
const RegisterAdmin = () => {
  ...
  const register = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:3000/api/usersDetails/register", userData);
      ...
      <Form className="form" onFinish={register}>
        ...
        <div className="flex">
          <label className='w-full'>
            <Form.Item name="name" rules={[{ required: true }]} label="Username">
        ...
        <label>
          <Form.Item name="password2" rules={[{ required: true }]} label="Confirm Password">
        ...
        <label>
          <Form.Item name="role" rules={[{ required: true }]} label="Role">
            <Select placeholder="Select a role" value={role} onChange={handleRoleChange}>
              <Option value="admin">Admin</Option>
              <Option value="user">User</Option>
            </Select>
          </Form.Item>
        </label>
        <Button type="primary" htmlType="submit" className="submit" loading={isLoading}>Register</Button>
        <div className='flex items-center justify-center'>
        <p className="signin">Already have an account? <Link to="/login">Signin</Link></p>
        <p className='signin'><Link to='/'>Home</Link></p>
        </div>
      </Form>
    </div>
  );
